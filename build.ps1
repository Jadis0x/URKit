...
<#
    URKit build driver. Wraps the CMakePresets.json presets (clang-debug,
    clang-release, msvc-debug, msvc-release) and takes care of locating a
    Visual Studio toolchain so cl.exe / link.exe / the Windows SDK are on
    PATH even when this isn't run from a Developer Prompt.
#>
[CmdletBinding()]
param(
...
Write-Host "== URKit build ($Compiler, $Config) ==" -ForegroundColor Cyan
