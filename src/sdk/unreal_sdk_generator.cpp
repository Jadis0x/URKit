...
std::string UnrealSdkReadme(const std::string &details) {
    ...
    out << "# URKit - Unreal SDK\n\n"
        ...
        << "Set `DumpTypes=1` under `[Unreal]` in the game's `URKit_config.ini` and play: every map that loads "
           ...
           "## Hooking a function\n\n"
           "```cpp\n"
           "static URK::unreal::FunctionHook damage = URK::unreal::hook(\n"
           "    URK::unreal::find(\"Actor\"), \"ReceiveAnyDamage\",\n"
           "    [](URK::unreal::HookedCall &call) {  // before: false skips the body\n"
           "        // What the actor's Blueprint damage event receives.\n"
           "        call.set<float>(\"Damage\", call.get<float>(\"Damage\").value_or(0) * 2);\n"
           "        return true;\n"
           "    },\n"
           "    [](URK::unreal::HookedCall &call) { /* after: the return value is in the frame */ });\n"
           "```\n\n"
