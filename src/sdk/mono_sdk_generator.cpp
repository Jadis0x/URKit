...
std::string MonoRuntimeModule() {
    return R"URKMONO(#pragma once
...
inline bool has_field(const URK_MonoApi *a, std::size_t end) {
    return URK::has_runtime_capability(URK::runtime_cap_mono_api) && a && a->version >= URK_MONO_API_VERSION &&
           a->size >= end;
}
#define URK_MONO_HAS(member) (has_field(a, offsetof(URK_MonoApi, member) + sizeof(a->member)))
inline bool init(const URK::ModContext *ctx) {
    URK::set_context(ctx);
    ApiSlot() = nullptr;
    if (!ctx || ctx->version < URK_SDK_VERSION || ctx->size < sizeof(URK_ModContext) ||
        ctx->runtimeBackend != URK::runtime_backend_mono || !URK::has_runtime_capability(URK::runtime_cap_mono_api) ||
        !ctx->mono || ctx->mono->version < URK_MONO_API_VERSION || ctx->mono->size < sizeof(URK_MonoApi))
        return false;
    ApiSlot() = ctx->mono;
    return true;
}
...
inline bool attach_current_thread() {
    const auto *a = api();
    return a && URK_MONO_HAS(attach_current_thread) && a->attach_current_thread ? a->attach_current_thread() != 0
                                                                                : false;
}
inline const Image *find_image(const char *image) {
    const auto *a = api();
    return a && URK_MONO_HAS(find_image) && a->find_image ? static_cast<const Image *>(a->find_image(image)) : nullptr;
}
...
inline const Method *resolve_method(const char *image, const char *namespc, const char *klass, const char *name,
                                    int argc = -1) {
    const auto *a = api();
    return a && URK_MONO_HAS(find_method) && a->find_method
               ? static_cast<const Method *>(a->find_method(image, namespc, klass, name, argc))
               : nullptr;
}
inline const Method *resolve_method_exact(const char *image, const char *namespc, const char *klass, const char *name,
    ...
    return a && URK_MONO_HAS(find_method_exact) && a->find_method_exact
               ? static_cast<const Method *>(
                     a->find_method_exact(image, namespc, klass, name, parameter_types, parameter_count))
               : nullptr;
}
inline const Field *resolve_field(const char *image, const char *namespc, const char *klass, const char *name) {
    ...
    return a && URK_MONO_HAS(find_field) && a->find_field
               ? static_cast<const Field *>(a->find_field(image, namespc, klass, name))
               : nullptr;
}
inline void *compile_method(const Method *method) {
    const auto *a = api();
    return a && URK_MONO_HAS(compile_method) && a->compile_method ? a->compile_method(method) : nullptr;
}
inline void *method_get_object(const Method *method, const Class *) {
    const auto *a = api();
    return a && URK_MONO_HAS(method_get_object) && a->method_get_object ? a->method_get_object(method) : nullptr;
}
inline void *value_box(const Class *klass, void *data) {
    const auto *a = api();
    return a && URK_MONO_HAS(value_box) && a->value_box ? a->value_box(klass, data) : nullptr;
}
inline int runtime_invoke(const Method *method, Object *object, void **params, void **result = nullptr,
                          void **exception = nullptr, std::uint32_t *native_exception = nullptr) {
    const auto *a = api();
    return a && URK_MONO_HAS(runtime_invoke) && a->runtime_invoke
               ? a->runtime_invoke(method, object, params, result, exception, native_exception)
               : 0;
}
inline String *string_new(const char *utf8) {
    const auto *a = api();
    return a && URK_MONO_HAS(new_string) && a->new_string ? static_cast<String *>(a->new_string(utf8)) : nullptr;
}
...
inline std::size_t array_length(Array *array) {
    const auto *a = api();
    return a && URK_MONO_HAS(array_length) && a->array_length ? a->array_length(array) : 0;
}
inline void *array_address(Array *array, int element_size, std::size_t index) {
    const auto *a = api();
    return a && URK_MONO_HAS(array_address) && a->array_address ? a->array_address(array, element_size, index)
                                                                : nullptr;
}
...
inline bool array_set_ref(Array *array, std::size_t index, void *value) {
    const auto *a = api();
    return a && URK_MONO_HAS(array_set_ref) && a->array_set_ref ? a->array_set_ref(array, index, value) != 0 : false;
}
inline bool has_array_set_ref() {
    const auto *a = api();
    return a && URK_MONO_HAS(array_set_ref) && a->array_set_ref;
}
inline bool object_class_name(Object *object, char *output, std::size_t output_size) {
    const auto *a = api();
...
inline void *object_unbox(Object *object) {
    const auto *a = api();
    return a && URK_MONO_HAS(object_unbox) && a->object_unbox ? a->object_unbox(object) : nullptr;
}
inline Object *object_new(const Class *klass) {
    const auto *a = api();
    return a && URK_MONO_HAS(object_new) && a->object_new ? static_cast<Object *>(a->object_new(klass)) : nullptr;
}
inline Object *type_get_object(const Type *type) {
    const auto *a = api();
    return a && URK_MONO_HAS(type_get_object) && a->type_get_object ? static_cast<Object *>(a->type_get_object(type))
                                                                    : nullptr;
}
inline void runtime_object_init(Object *object) {
    const auto *a = api();
    if (a && URK_MONO_HAS(runtime_object_init) && a->runtime_object_init && object)
        a->runtime_object_init(object);
}
inline bool string_to_utf8(String *string, char *output, std::size_t output_size) {
    const auto *a = api();
    return a && URK_MONO_HAS(string_to_utf8) && a->string_to_utf8 ? a->string_to_utf8(string, output, output_size) != 0
                                                                  : false;
}
inline std::size_t string_length(String *string) {
    const auto *a = api();
    return a && URK_MONO_HAS(string_length) && a->string_length ? a->string_length(string) : 0;
}
inline const char *class_get_name(const Class *klass) {
    const auto *a = api();
    return a && URK_MONO_HAS(class_get_name) && a->class_get_name ? a->class_get_name(klass) : nullptr;
}
inline const char *class_get_namespace(const Class *klass) {
    const auto *a = api();
    return a && URK_MONO_HAS(class_get_namespace) && a->class_get_namespace ? a->class_get_namespace(klass) : nullptr;
}
...
inline std::uint32_t class_get_flags(const Class *klass) {
    const auto *a = api();
    return a && URK_MONO_HAS(class_get_flags) && a->class_get_flags ? a->class_get_flags(klass) : 0;
}
...
inline const char *field_get_name(const Field *field) {
    const auto *a = api();
    return a && URK_MONO_HAS(field_get_name) && a->field_get_name ? a->field_get_name(field) : nullptr;
}
inline const Type *field_get_type(const Field *field) {
    const auto *a = api();
    return a && URK_MONO_HAS(field_get_type) && a->field_get_type ? static_cast<const Type *>(a->field_get_type(field))
                                                                  : nullptr;
}
inline std::uint32_t field_get_offset(const Field *field) {
    const auto *a = api();
    return a && URK_MONO_HAS(field_get_offset) && a->field_get_offset ? a->field_get_offset(field) : 0;
}
inline std::uint32_t field_get_flags(const Field *field) {
    const auto *a = api();
    return a && URK_MONO_HAS(field_get_flags) && a->field_get_flags ? a->field_get_flags(field) : 0;
}
inline const char *last_error() {
    const auto *a = api();
    return a && URK_MONO_HAS(last_error) && a->last_error ? a->last_error() : nullptr;
}
inline const void *domain_get() {
    const auto *a = api();
    return a && URK_MONO_HAS(domain_get) && a->domain_get ? a->domain_get() : nullptr;
}
inline const void *root_domain_get() {
    const auto *a = api();
    return a && URK_MONO_HAS(root_domain_get) && a->root_domain_get ? a->root_domain_get() : nullptr;
}
...
inline const char *image_get_name(const Image *image) {
    const auto *a = api();
    return a && URK_MONO_HAS(image_get_name) && a->image_get_name ? a->image_get_name(image) : nullptr;
}
inline const char *image_get_filename(const Image *image) {
    const auto *a = api();
    return a && URK_MONO_HAS(image_get_filename) && a->image_get_filename ? a->image_get_filename(image) : nullptr;
}
inline int image_get_table_rows(const Image *image, int table_id) {
    const auto *a = api();
    return a && URK_MONO_HAS(image_get_table_rows) && a->image_get_table_rows ? a->image_get_table_rows(image, table_id)
                                                                              : 0;
}
...
inline const Type *class_get_type(const Class *klass) {
    const auto *a = api();
    return a && URK_MONO_HAS(class_get_type) && a->class_get_type ? static_cast<const Type *>(a->class_get_type(klass))
                                                                  : nullptr;
}
inline bool class_is_valuetype(const Class *klass) {
    const auto *a = api();
    return a && URK_MONO_HAS(class_is_valuetype) && a->class_is_valuetype ? a->class_is_valuetype(klass) != 0 : false;
}
inline bool class_is_enum(const Class *klass) {
    const auto *a = api();
    return a && URK_MONO_HAS(class_is_enum) && a->class_is_enum ? a->class_is_enum(klass) != 0 : false;
}
...
inline const char *method_get_name(const Method *method) {
    const auto *a = api();
    return a && URK_MONO_HAS(method_get_name) && a->method_get_name ? a->method_get_name(method) : nullptr;
}
inline bool method_is_generic(const Method *method) {
    const auto *a = api();
    return a && URK_MONO_HAS(method_is_generic) && a->method_is_generic ? a->method_is_generic(method) != 0 : false;
}
inline std::uint32_t method_get_flags(const Method *method, std::uint32_t *iflags = nullptr) {
    const auto *a = api();
    return a && URK_MONO_HAS(method_get_flags) && a->method_get_flags ? a->method_get_flags(method, iflags) : 0;
}
...
inline std::uint32_t signature_get_param_count(const Signature *signature) {
    const auto *a = api();
    return a && URK_MONO_HAS(signature_get_param_count) && a->signature_get_param_count
               ? a->signature_get_param_count(signature)
               : 0;
}
inline const Type *signature_get_return_type(const Signature *signature) {
    const auto *a = api();
    return a && URK_MONO_HAS(signature_get_return_type) && a->signature_get_return_type
               ? static_cast<const Type *>(a->signature_get_return_type(signature))
               : nullptr;
}
inline const Type *signature_get_param(const Signature *signature, void **iterator) {
    const auto *a = api();
    return a && URK_MONO_HAS(signature_get_param) && a->signature_get_param
               ? static_cast<const Type *>(a->signature_get_param(signature, iterator))
               : nullptr;
}
...
inline const Type *method_get_param_type(const Method *method, std::uint32_t index) {
    const auto *a = api();
    return a && URK_MONO_HAS(method_get_param_type) && a->method_get_param_type
               ? static_cast<const Type *>(a->method_get_param_type(method, index))
               : nullptr;
}
inline const char *property_get_name(const Property *property) {
    const auto *a = api();
    return a && URK_MONO_HAS(property_get_name) && a->property_get_name ? a->property_get_name(property) : nullptr;
}
inline const Method *property_get_get_method(const Property *property) {
    const auto *a = api();
    return a && URK_MONO_HAS(property_get_get_method) && a->property_get_get_method
...
inline const Method *property_get_set_method(const Property *property) {
    const auto *a = api();
    return a && URK_MONO_HAS(property_get_set_method) && a->property_get_set_method
...
inline std::uint32_t property_get_flags(const Property *property) {
    const auto *a = api();
    return a && URK_MONO_HAS(property_get_flags) && a->property_get_flags ? a->property_get_flags(property) : 0;
}
inline bool field_get_value(Object *object, const Field *field, void *output) {
    const auto *a = api();
    return a && URK_MONO_HAS(field_get_value) && a->field_get_value ? a->field_get_value(object, field, output) != 0
                                                                    : false;
}
inline bool field_set_value(Object *object, const Field *field, void *value) {
    const auto *a = api();
    return a && URK_MONO_HAS(field_set_value) && a->field_set_value ? a->field_set_value(object, field, value) != 0
                                                                    : false;
}
inline bool field_static_get_value(const Class *klass, const Field *field, void *output) {
    const auto *a = api();
    return a && URK_MONO_HAS(field_static_get_value) && a->field_static_get_value
               ? a->field_static_get_value(klass, field, output) != 0
               : false;
}
inline bool field_static_set_value(const Class *klass, const Field *field, void *value) {
    const auto *a = api();
...
inline bool type_get_name(const Type *type, char *output, std::size_t output_size) {
    const auto *a = api();
    return a && URK_MONO_HAS(type_get_name) && a->type_get_name ? a->type_get_name(type, output, output_size) != 0
                                                                : false;
}
inline std::int32_t type_get_type(const Type *type) {
    const auto *a = api();
    return a && URK_MONO_HAS(type_get_type) && a->type_get_type ? a->type_get_type(type) : 0;
}
inline std::uint32_t type_get_attrs(const Type *type) {
    const auto *a = api();
    return a && URK_MONO_HAS(type_get_attrs) && a->type_get_attrs ? a->type_get_attrs(type) : 0;
}
inline const Class *type_get_class(const Type *type) {
    const auto *a = api();
    return a && URK_MONO_HAS(type_get_class) && a->type_get_class ? static_cast<const Class *>(a->type_get_class(type))
                                                                  : nullptr;
}
inline const void *thread_current() {
    const auto *a = api();
    return a && URK_MONO_HAS(thread_current) && a->thread_current ? a->thread_current() : nullptr;
}
...
inline std::string to_utf8(URK::mono::String *value, std::string_view fallback) {
    if (!value)
...
inline bool resolve_class(const char *image, const char *namespc, const char *name, const URK::mono::Class *&out) {
    out = URK::mono::find_class(image, namespc, name);
    return out != nullptr;
}
inline bool resolve_method(const char *image, const char *namespc, const char *klass, const char *name, int argc,
                           const URK::mono::Method *&out) {
    out = URK::mono::resolve_method(image, namespc, klass, name, argc);
    return out != nullptr;
}
inline bool resolve_method_exact(const char *image, const char *namespc, const char *klass, const char *name,
                                 const char *const *parameter_types, int parameter_count,
                                 const URK::mono::Method *&out) {
    out = URK::mono::resolve_method_exact(image, namespc, klass, name, parameter_types, parameter_count);
    return out != nullptr;
}
inline bool resolve_field(const char *image, const char *namespc, const char *klass, const char *name,
                          const URK::mono::Field *&out) {
    out = URK::mono::resolve_field(image, namespc, klass, name);
    return out != nullptr;
}

inline bool require_class(const char *image, const char *namespc, const char *name, const URK::mono::Class *&out,
                          DiagnosticSink sink = nullptr) {
    if (resolve_class(image, namespc, name, out))
        return true;
    char msg[512]{};
    std::snprintf(msg, sizeof(msg), "[URK Mono SDK] missing class: %s:%s.%s", image ? image : "",
...
inline bool require_method_exact(const char *image, const char *namespc, const char *klass, const char *name,
                                 ...
                                 const URK::mono::Method *&out, DiagnosticSink sink = nullptr) {
    if (resolve_method_exact(image, namespc, klass, name, parameter_types, parameter_count, out))
        return true;
    char params[384]{};
    std::size_t used = 0;
    for (int i = 0; i < parameter_count && used < sizeof(params); ++i) {
        const int written = std::snprintf(params + used, sizeof(params) - used, "%s%s", i ? ", " : "",
    ...
    char msg[1024]{};
    std::snprintf(msg, sizeof(msg), "[URK Mono SDK] missing/changed method: %s:%s.%s.%s(%s) -> %s", image ? image : "",
                  namespc ? namespc : "", klass ? klass : "", name ? name : "", params,
...
inline bool require_method_exact(const char *image, const char *namespc, const char *klass, const char *name,
                                 const char *const *parameter_types, int parameter_count, const URK::mono::Method *&out,
                                 DiagnosticSink sink = nullptr) {
    return require_method_exact(image, namespc, klass, name, parameter_types, parameter_count, "<unknown>", out, sink);
}
inline bool require_field(const char *image, const char *namespc, const char *klass, const char *name,
                          const URK::mono::Field *&out, DiagnosticSink sink = nullptr) {
    if (resolve_field(image, namespc, klass, name, out))
        return true;
    char msg[640]{};
    std::snprintf(msg, sizeof(msg), "[URK Mono SDK] missing/changed field: %s:%s.%s.%s", image ? image : "",
                  namespc ? namespc : "", klass ? klass : "", name ? name : "");
    emit(sink, msg);
    emit_last_error(sink);
    return false;
}
inline bool require_method(const char *image, const char *namespc, const char *klass, const char *name, int argc,
                           const URK::mono::Method *&out, DiagnosticSink sink = nullptr) {
    if (resolve_method(image, namespc, klass, name, argc, out))
        return true;
    char msg[640]{};
    std::snprintf(msg, sizeof(msg), "[URK Mono SDK] missing method: %s:%s.%s.%s/%d", image ? image : "",
                  namespc ? namespc : "", klass ? klass : "", name ? name : "", argc);
    emit(sink, msg);
    emit_last_error(sink);
    return false;
}
inline bool require_property(const URK::mono::Class *klass_handle, const char *name, const URK::mono::Property *&out,
                             DiagnosticSink sink = nullptr) {
    void *it = nullptr;
    while ((out = URK::mono::class_get_properties(klass_handle, &it)) != nullptr) {
        const char *n = URK::mono::property_get_name(out);
        if (n && name && std::strcmp(n, name) == 0)
            return true;
    }
    char msg[512]{};
    std::snprintf(msg, sizeof(msg), "[URK Mono SDK] missing property: %s", name ? name : "");
    emit(sink, msg);
    emit_last_error(sink);
    return false;
}
...
std::string MonoSdkReadme(const std::string &details) {
    std::ostringstream out;
    out << "# URKit - Mono SDK\n\n"
        << "This folder is generated for Mono native header-only mods. "
           "Generated SDK files use `.h`.\n\n"
        << "Mono and IL2CPP generated projects use runtime API helpers. "
           "No offline metadata or dump-generated headers are emitted. Mono helpers "
           "resolve classes, methods, fields, properties, and objects through "
           "the public runtime API at mod runtime. Use "
           "`URK::mono::helpers::require_*` helpers to log diagnostics for "
           "missing or changed members.\n\n"
        << details;
    return out.str();
}
...
bool HasUsableOutput(const std::string &directory) {
    std::error_code ec;
    const fs::path monoRoot(directory);
    const fs::path files[] = {monoRoot / "mono_runtime.h", monoRoot / "mono_helpers.h", monoRoot / "README.md"};
...
bool Generate(const std::string &directory, const std::string &reportDetails, std::string *error) {
    ...
    plan.files = {
        {"mono_runtime.h", {}, MonoRuntimeModule(), mpg::OutputFilePolicy::GeneratedOverwrite, true, true},
        {"mono_helpers.h", {}, MonoHelpersModule(), mpg::OutputFilePolicy::GeneratedOverwrite, true, true},
        {"README.md", {}, MonoSdkReadme(reportDetails), mpg::OutputFilePolicy::GeneratedOverwrite, true, false},
    };

    sdk::OutputResult output;
    if (!sdk::PublishOutputPlanAtomically(plan, &output, error))
        return false;
...
bool GenerateModProject(const std::string &projectRoot, const std::string &monoSdkRoot,
    ...
    const fs::path monoRoot(monoSdkRoot);
    const auto profile = mpg::MonoBackendProfile();
    fs::path sdkHeaderPath;
    if (!commonIncludeRoot.empty())
        sdkHeaderPath = fs::path(commonIncludeRoot) / "mod_sdk.h";

    if (!HasUsableOutput(monoRoot.string())) {
        if (error)
            *error = "Mono SDK is missing or incomplete; generate Mono SDK before project";
        return false;
    }
    ...
    backendFiles.files = {
        {profile.sdkSubdirectory / "mono_runtime.h",
         monoRoot / "mono_runtime.h",
         {},
         mpg::OutputFilePolicy::GeneratedOverwrite,
         true,
         true},
        {profile.sdkSubdirectory / "mono_helpers.h",
         monoRoot / "mono_helpers.h",
         {},
         mpg::OutputFilePolicy::GeneratedOverwrite,
         true,
         true},
    ...
    const fs::path modsPath = fs::path(gameDirectory) / modsDirectory;
    auto options = mpg::MakeModuleProjectOptions(profile, root, project, sdkHeaderPath);
    options.deployDirectory = modsPath.string();
    options.enableLocalization = enableLocalization;
    options.readmeExtraLayoutLines = {
        "Mono and IL2CPP generated projects use runtime API helpers. No offline "
        "metadata or dump-generated headers are emitted.",
    };
