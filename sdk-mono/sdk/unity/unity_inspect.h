...
inline std::string type_name(const void* type) { char out[256]{}; return URK::mono::type_get_name(static_cast<const URK::mono::Type*>(type), out, sizeof(out)) ? std::string(out) : std::string{}; }
inline TypeInfo DescribeClass(const void* klass) {
    TypeInfo out{};
    out.handle = klass;
    if (!klass) return out;
    const auto* k = static_cast<const URK::mono::Class*>(klass);
    const char* ns = URK::mono::class_get_namespace(k);
    const char* name = URK::mono::class_get_name(k);
    out.namespc = ns ? ns : "";
    out.name = name ? name : "";
    out.full_name = out.namespc.empty() ? out.name : out.namespc + "." + out.name;
    out.flags = URK::mono::class_get_flags(k);
    out.is_value_type = URK::mono::class_is_valuetype(k);
    out.is_enum = URK::mono::class_is_enum(k);
    return out;
}
inline TypeInfo DescribeType(const void* type) {
    return DescribeClass(URK::mono::type_get_class(static_cast<const URK::mono::Type*>(type)));
}
...
inline std::vector<FieldInfo> fields_from_class(const URK::mono::Class* klass, bool includeInherited) {
    std::vector<FieldInfo> out;
    for (const void* current = klass; current; current = includeInherited ? URK::mono::class_get_parent(static_cast<const URK::mono::Class*>(current)) : nullptr) {
        const TypeInfo declaring = DescribeClass(current);
        void* it = nullptr;
        while (const auto* field = URK::mono::class_get_fields(static_cast<const URK::mono::Class*>(current), &it)) {
            FieldInfo info{};
            info.handle = field;
            info.declaring_type = declaring;
            const char* name = URK::mono::field_get_name(field);
            info.name = name ? name : "";
            const void* fieldType = URK::mono::field_get_type(field);
            const TypeInfo fieldTypeInfo = DescribeType(fieldType);
            info.type = fieldType;
            info.type_name = type_name(fieldType);
            info.flags = URK::mono::field_get_flags(field);
...
inline std::vector<FieldInfo> Fields(TypeRef type, bool includeInherited = true) {
    ...
    return fields_from_class(static_cast<const URK::mono::Class*>(klass), includeInherited);
}
...
inline MethodInfo method_info(const URK::mono::Method* method, TypeInfo declaring) {
    MethodInfo info{};
    info.handle = method;
    info.declaring_type = std::move(declaring);
    const char* name = URK::mono::method_get_name(method);
    info.name = name ? name : "";
    info.flags = URK::mono::method_get_flags(method, &info.iflags);
    info.is_static = (info.flags & kStaticMemberFlag) != 0;
    info.return_type_handle = URK::mono::method_get_return_type(method);
    info.return_type = type_name(info.return_type_handle);
    const auto* sig = URK::mono::method_signature(method);
...
inline std::vector<MethodInfo> methods_from_class(const URK::mono::Class* klass, bool includeInherited) {
    std::vector<MethodInfo> out;
    for (const void* current = klass; current; current = includeInherited ? URK::mono::class_get_parent(static_cast<const URK::mono::Class*>(current)) : nullptr) {
        const TypeInfo declaring = DescribeClass(current);
        void* it = nullptr;
        while (const auto* method = URK::mono::class_get_methods(static_cast<const URK::mono::Class*>(current), &it))
...
inline std::vector<MethodInfo> Methods(TypeRef type, bool includeInherited = true) {
    ...
    return methods_from_class(static_cast<const URK::mono::Class*>(klass), includeInherited);
}
...
inline PropertyInfo property_info(const URK::mono::Property* property, TypeInfo declaring) {
    PropertyInfo info{};
    info.handle = property;
    info.declaring_type = std::move(declaring);
    const char* name = URK::mono::property_get_name(property);
    info.name = name ? name : "";
    info.flags = URK::mono::property_get_flags(property);
    info.get_method = URK::mono::property_get_get_method(property);
    info.set_method = URK::mono::property_get_set_method(property);
    info.can_read = info.get_method != nullptr;
    info.can_write = info.set_method != nullptr;
    const void* propertyType = info.get_method ? URK::mono::method_get_return_type(static_cast<const URK::mono::Method*>(info.get_method))
                                               : (info.set_method ? URK::mono::method_get_param_type(static_cast<const URK::mono::Method*>(info.set_method), 0) : nullptr);
...
inline std::vector<PropertyInfo> properties_from_class(const URK::mono::Class* klass, bool includeInherited) {
    std::vector<PropertyInfo> out;
    for (const void* current = klass; current; current = includeInherited ? URK::mono::class_get_parent(static_cast<const URK::mono::Class*>(current)) : nullptr) {
        const TypeInfo declaring = DescribeClass(current);
        void* it = nullptr;
        while (const auto* property = URK::mono::class_get_properties(static_cast<const URK::mono::Class*>(current), &it))
...
inline std::vector<PropertyInfo> Properties(TypeRef type, bool includeInherited = true) {
    ...
    return properties_from_class(static_cast<const URK::mono::Class*>(klass), includeInherited);
}
...
inline std::string enum_underlying_type_name(const void* type) {
    const void* klass = type ? URK::mono::type_get_class(static_cast<const URK::mono::Type*>(type)) : nullptr;
    if (!klass) return {};
    void* it = nullptr;
    while (const auto* field = URK::mono::class_get_fields(static_cast<const URK::mono::Class*>(klass), &it)) {
        const char* name = URK::mono::field_get_name(field);
        if (name && std::string_view{name} == "value__") return type_name(URK::mono::field_get_type(field));
    }
    return {};
}
