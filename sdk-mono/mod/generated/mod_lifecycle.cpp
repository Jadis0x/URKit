...
bool validate_required_backend(const URK_ModContext* ctx) {
  ...
  if (ctx->runtimeBackend != URK::runtime_backend_mono) {
    ModLog::error("required backend mismatch: generated for Mono but loader runtimeBackend=%u; refusing to initialize", ctx->runtimeBackend);
    return false;
  }
  ...
  if (ctx->mono->version < URK_MONO_API_VERSION || ctx->mono->size < sizeof(*ctx->mono)) {
    ModLog::error("required Mono API table is incompatible: version=%d size=%u", ctx->mono->version, ctx->mono->size);
    return false;
  }
  return true;
}
