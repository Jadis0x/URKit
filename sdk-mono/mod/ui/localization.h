...
inline const Dictionary& english() {
    static const Dictionary values = {
        {"about.title", "About"}, {"about.default_description", "Small runtime menu for this mod."},
        {"about.author", "Author"}, {"about.version", "Version"}, {"about.url", "URL"},
        {"about.project_url", "Project URL"}, {"about.social", "Social"}, {"about.github", "GitHub"},
        {"menu.about", "About"}, {"menu.config", "Config"},
        {"config.controls", "Controls"},
        {"config.show_menu", "Show menu"}, {"config.toggle_key", "Toggle key"},
        {"config.localization", "Localization"},
        {"config.enable_localization", "Enable localization"},
        {"config.language", "Language"},
        {"widget.enabled", "Enabled"}, {"widget.disabled", "Disabled"},
        {"example.key_code", "Key code: {code}"},
    };
    return values;
}

inline Dictionary& overrides() { static Dictionary values; return values; }
inline std::vector<std::string>& languages() { static std::vector<std::string> values{"en"}; return values; }
inline std::string& current_language() { static std::string value = "en"; return value; }
inline std::string& last_error() { static std::string value; return value; }
inline bool& initialized() { static bool value = false; return value; }

inline void log_warning(const std::string& message) {
    const URK::ModContext* ctx = URK::context();
    if (ctx && ctx->Log) ctx->Log("[%s][localization] %s", ModConfig::display_name, message.c_str());
}
...
inline bool parse_string(const std::string& source, size_t& pos, std::string& output, std::string& error) {
    if (pos >= source.size() || source[pos++] != '"') { error = "expected JSON string"; return false; }
    output.clear();
    while (pos < source.size()) {
        const char ch = source[pos++];
        if (ch == '"') return true;
        if (static_cast<unsigned char>(ch) < 0x20) { error = "control character in JSON string"; return false; }
        if (ch != '\\') { output.push_back(ch); continue; }
        if (pos >= source.size()) { error = "unterminated JSON escape"; return false; }
        ...
        case 'u': {
            if (pos + 4 > source.size()) { error = "truncated Unicode escape"; return false; }
            unsigned value = 0;
            for (int i = 0; i < 4; ++i) {
            ...
            if (value >= 0xd800 && value <= 0xdfff) { error = "Unicode surrogate escapes are unsupported"; return false; }
            append_unicode(output, value);
            break;
        }
        default: error = "invalid JSON escape"; return false;
        }
    }
...
inline bool parse_dictionary(const std::string& source, Dictionary& output, std::string& error) {
    ...
    if (pos >= source.size() || source[pos++] != '{') { error = "root must be an object"; return false; }
    output.clear();
    skip_whitespace(source, pos);
    while (pos < source.size() && source[pos] != '}') {
        std::string key;
        std::string value;
        if (!parse_string(source, pos, key, error)) return false;
        skip_whitespace(source, pos);
        if (pos >= source.size() || source[pos++] != ':') { error = "expected ':' after JSON key"; return false; }
        skip_whitespace(source, pos);
        if (!parse_string(source, pos, value, error)) { error = "all localization values must be strings: " + error; return false; }
        output[std::move(key)] = std::move(value);
        skip_whitespace(source, pos);
        if (pos < source.size() && source[pos] == ',') { ++pos; skip_whitespace(source, pos); continue; }
        if (pos >= source.size() || source[pos] != '}') { error = "expected ',' or '}' in JSON object"; return false; }
    }
    if (pos >= source.size() || source[pos++] != '}') { error = "unterminated JSON object"; return false; }
    skip_whitespace(source, pos);
    if (pos != source.size()) { error = "unexpected content after JSON object"; return false; }
    return true;
}
...
inline std::filesystem::path locale_directory() {
    ...
    if (length == 0 || length == path.size()) { report_error("cannot read the mod DLL path"); return {}; }
...
inline bool read_language(const std::string& language, Dictionary& output, std::string& error) {
    ...
    if (!input) { error = "cannot open locale file: " + path.string(); return false; }
...
inline bool load_language(const char* language) {
    if (!language || !language[0] || std::string(language) == "en") {
        overrides().clear(); current_language() = "en"; last_error().clear(); return true;
    }
    Dictionary parsed;
    std::string error;
    if (!read_language(language, parsed, error)) { report_error(error); return false; }
...
inline void initialize() {
    if (initialized() || !ModConfig::enable_localization) return;
    initialized() = true;
    ...
    if (error) { report_error("cannot enumerate locale directory: " + error.message()); return; }
    for (const auto& entry : entries) {
        std::error_code entry_error;
        if (!entry.is_regular_file(entry_error)) {
            if (entry_error) report_error("cannot inspect locale entry: " + entry_error.message());
            continue;
        }
        if (entry.path().extension() != ".json") continue;
        const std::string language = entry.path().stem().string();
        if (language.empty() || language == "en") continue;
        Dictionary parsed;
...
inline const std::vector<std::string>& available_languages() { initialize(); return languages(); }
inline const char* active_language() { initialize(); return current_language().c_str(); }
inline const char* last_error_message() { initialize(); return last_error().c_str(); }
...
inline const char* translate(const char* key) {
    ...
    if (override_it != overrides().end()) return override_it->second.c_str();
...
inline std::string format(const char* key, std::initializer_list<Replacement> replacements) {
    ...
    for (const Replacement& replacement : replacements) {
        if (!replacement.name || !replacement.name[0]) continue;
